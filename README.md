...
2. Ищет контакт с таким адресом в Exchange. При запуске с `-Staged` пользователь добавляется в группы контакта, а сам контакт остаётся. При финальном запуске с `-Force` контакт удаляется.
3. Проверяет mailbox в Exchange; если нет — **Enable-Mailbox** (при `-Staged` отключает учётную запись и скрывает её из адресных списков) и пауза 60 сек.
...
Такой запуск создаёт и отключает mailbox, скрывает его из адресных списков и переносит пользователя в группы контакта, оставляя сам контакт. Позже выполните миграцию повторно без `-Staged` (и при необходимости с `-Force`) для удаления контакта и завершения переноса.
